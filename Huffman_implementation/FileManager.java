...
public class FileManager {
    static String NAME = "StudentAlex";

    static private void readArchiveHeader(String filePath, Archive dataArc) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            IntStream.range(0, recoveryMapSize).forEach(index -> {
                try {
                    char symbol;
                    String code;
        ...
        dataArc.dataOffset = headerData.toString().getBytes().length;
        dataArc.recoveryMap = recoveryMap;
        dataArc.originalFileName = originalFileName;
    }
    ...
    static private void readArchive(String filePath, Archive dataArc) {
        ...
        try {
            ...
            fileInputStream.skip(dataArc.dataOffset);
        ...
        dataArc.originalLength = originalLength;
        dataArc.compressedData = compressed.substring(8, compressed.length() - additionalZerosNumber);
    }

    static public Archive readArchive(String filePath) {
        Archive dataArc = new Archive();

        readArchiveHeader(filePath, dataArc);
        readArchive(filePath, dataArc);
    ...
    static public void test(Archive archive, byte[] result) {
        System.out.println("Archive.java length: " + archive.originalLength);
        System.out.println("Original file name: " + archive.originalFileName);
        System.out.println("Original file length: " + result.length);

        if (result.length > 0) {
            System.out.println("Compress ratio: " + (100 * archive.originalLength / result.length) + "%");
        }

        System.out.println("Code map:");
        archive.recoveryMap.forEach((code, symbol) -> {
